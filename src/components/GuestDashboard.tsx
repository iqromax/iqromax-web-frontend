...
export const GuestDashboard = () => {
  ...
  return (
    <div className="space-y-12 sm:space-y-20 pb-10">
      {/* ════════════ HERO ════════════ */}
      <section className="relative -mx-3 xs:-mx-4 overflow-hidden rounded-b-[2rem] sm:rounded-[2rem] sm:mx-0">
        {/* Dark green background with grid pattern */}
        <div className="absolute inset-0 bg-gradient-to-br from-[#0a2818] via-[#0f3a22] to-[#06180e]" />
        <div
        ...
        <div className="absolute top-10 left-8 text-emerald-400/10 text-6xl font-bold select-none">+</div>
        <div className="absolute top-32 right-16 text-emerald-400/10 text-5xl font-bold select-none">×</div>
        <div className="absolute bottom-20 left-20 text-emerald-400/10 text-7xl font-bold select-none">÷</div>
        <div className="absolute bottom-32 right-10 text-emerald-400/10 text-6xl font-bold select-none">=</div>

        <div className="relative grid lg:grid-cols-[1.1fr_1fr] gap-6 lg:gap-8 px-5 sm:px-8 lg:px-12 py-8 sm:py-12 lg:py-16">
          {/* Left: Headline + CTA */}
          <div className="flex flex-col justify-center">
            <div className="inline-flex items-center gap-2 self-start px-3 py-1.5 rounded-full bg-emerald-500/15 border border-emerald-400/30 text-emerald-300 text-xs font-semibold mb-4">
              <Sparkles className="h-3.5 w-3.5" />
              IQROMAX
            </div>
            <h1 className="text-[28px] xs:text-3xl sm:text-4xl lg:text-5xl font-black text-white leading-[1.1] mb-4">
              IQROMAX —{' '}
              <span className="text-emerald-400">bolalar uchun tez hisoblash</span> va{' '}
              <span className="text-emerald-400">aqliy rivojlanish</span> platformasi
            </h1>
            <p className="text-sm sm:text-base text-emerald-50/70 leading-relaxed mb-6 max-w-xl">
              Yapon abakus metodikasi, qiziqarli o'yinlar va zamonaviy texnologiyalar yordamida bolangizning aqliy
              salohiyatini oching va natijasini real vaqtda kuzating.
            </p>
            <div className="flex flex-wrap gap-3 mb-5">
              <Button
                size="lg"
                onClick={() => navigate('/auth')}
                className="h-12 px-6 rounded-full bg-emerald-500 hover:bg-emerald-400 text-white font-bold shadow-[0_8px_30px_-6px_rgba(16,185,129,0.5)]"
              >
              ...
              <Button
                ...
                variant="outline"
                onClick={() => navigate('/auth')}
                className="h-12 px-6 rounded-full bg-white/5 border-white/20 text-white hover:bg-white/10 hover:text-white font-bold backdrop-blur"
              >
                <Play className="h-4 w-4" /> Demo ko'rish
              </Button>
            </div>
            <div className="flex items-center gap-3 text-emerald-50/70 text-xs">
              <div className="flex -space-x-2">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-7 w-7 rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 border-2 border-[#0a2818] flex items-center justify-center text-[10px] font-bold text-white"
                  >
                    {['A', 'B', 'M'][i - 1]}
                  </div>
                ))}
              ...
              <span>
                <span className="font-bold text-emerald-300">{fmt(stats.total_users || 10000)}</span> o'quvchi bizga ishonadi
              </span>
            </div>
          </div>

          {/* Right: Boy + abacus image + level card + chart */}
          <div className="relative min-h-[280px] lg:min-h-[400px]">
            <div className="absolute inset-0 bg-gradient-radial from-emerald-500/20 via-transparent to-transparent rounded-2xl" />
            <img
              src={heroKids}
            ...
            />
            {/* Level card overlay */}
            <div className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-[#0a2818]/85 backdrop-blur-md border border-emerald-400/30 rounded-2xl p-3 sm:p-4 shadow-2xl w-[140px] sm:w-[170px]">
              <div className="flex items-center gap-2 mb-2">
                ...
                <div>
                  <p className="text-[10px] text-emerald-300/70">Level 7</p>
                  <p className="text-[11px] font-bold text-white">XP 650 / 1200</p>
                </div>
                <Trophy className="h-4 w-4 text-amber-400 ml-auto" />
              </div>
              <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-emerald-400 to-emerald-300 rounded-full" style={{ width: '54%' }} />
              </div>
              <p className="text-[9px] text-emerald-300/60 mt-1">54%</p>
            </div>

            {/* Weekly progress card */}
            <div className="absolute bottom-3 right-3 sm:bottom-4 sm:right-4 bg-[#0a2818]/85 backdrop-blur-md border border-emerald-400/30 rounded-2xl p-3 sm:p-4 shadow-2xl w-[180px] sm:w-[220px]">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[11px] font-bold text-white">Haftalik progress</p>
                <span className="text-[10px] text-emerald-400 font-bold">+28%</span>
              </div>
              <svg viewBox="0 0 200 60" className="w-full h-12">
                <polyline
                  points={weekData.map((v, i) => `${i * 33},${60 - v * 0.5}`).join(' ')}
                  fill="none"
                ...
                />
                {weekData.map((v, i) => (
                  <circle key={i} cx={i * 33} cy={60 - v * 0.5} r="2.5" fill="rgb(52, 211, 153)" />
                ))}
              </svg>
